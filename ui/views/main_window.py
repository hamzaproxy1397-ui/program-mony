...
class MainApplication:
    ...
    def create_main_window(self) -> None:
        ...
        # استخدام الحجم المحسن للنافذة الرئيسية
        try:
            # استخدام الحجم المحسن من الإعدادات
            ...
            # إذا كان حجم الشاشة أصغر من الحجم المطلوب، استخدم حجم الشاشة
            if screen_width < main_width or screen_height < main_height:
                main_width = min(main_width, screen_width - 100)  # ترك هامش 50px من كل جانب
            ...
            print(f"تحذير: فشل في تطبيق الحجم المحسن، استخدام الحجم الافتراضي: {e}")
            # كبديل احتياطي - استخدام حجم محسن ثابت
            self.main_window.geometry("1850x1000+50+50")
    ...
    def create_main_content(self):
        """إنشاء محتوى النافذة الرئيسية مطابق للصورة"""
        # الإطار الرئيسي بخلفية رمادية داكنة
        ...
        content_frame = ctk.CTkFrame(main_frame, fg_color="#2C2C2C")
    ...
    def create_top_menu_bar(self, parent):
        """إنشاء شريط القائمة العلوي مطابق للصورة"""
        # الشريط العلوي بلون رمادي فاتح - محسن
        menu_bar = ctk.CTkFrame(parent, height=40, fg_color="#F5F5F5")  # زيادة الارتفاع (+5px)
        ...
        search_entry = ctk.CTkEntry(
            ...
            width=240,  # زيادة العرض (+40px)
            height=30,  # زيادة الارتفاع (+5px)
        ...
        # القائمة الرئيسية في الوسط والجانب الأيمن
        menu_frame = ctk.CTkFrame(menu_bar, fg_color="transparent")
        menu_frame.pack(side="right", padx=20, pady=5)

        # عناصر القائمة من اليمين إلى اليسار مطابقة للصورة
        menu_items = [
        ...
        for item_text, item_command in menu_items:
    ...
    def create_green_top_bar(self, parent):
        """إنشاء الشريط الأخضر العلوي مع الأيقونات مطابق للصورة - محسن"""
        green_bar = ctk.CTkFrame(parent, height=170, fg_color="#2E8B57")  # تكبير الشريط الأخضر قليلاً
        green_bar.pack(fill="x", pady=(0, 0))
        green_bar.pack_propagate(False)

        # الأيقونات الست في الشريط الأخضر
        icons_frame = ctk.CTkFrame(green_bar, fg_color="transparent")
        icons_frame.pack(expand=True, fill="both", padx=30, pady=(10, 20))  # تحسين المسافات مع الشريط الأكبر

        # أيقونات الشريط الأخضر - محدثة حسب المطلوب
        green_icons = [
            ("assets/icons/13.ico", "الموظفين"),
            ("assets/icons/43.ico", "المحاسبة"),
            ("assets/icons/40.ico", "الحسابات"),
            ("assets/icons/50.ico", "الخزينة"),
            ("assets/icons/9.ico", "الفواتير"),
            ("assets/icons/4.ico", "التقارير"),
        ]

        for i, (icon_path, text) in enumerate(green_icons):
            # إطار الأيقونة
            icon_frame = ctk.CTkFrame(
                icons_frame,
                width=130,  # حجم معقول ومتوازن
                height=140,  # زيادة الارتفاع ليتناسب مع الشريط الأكبر
                fg_color="transparent",
                corner_radius=0
            )
            ...
            if os.path.exists(icon_path):
                icon_image = self.load_icon_image_no_bg(icon_path, (65, 65))  # تصغير حجم الأيقونات قليلاً
                if icon_image:
                    ...
                    icon_label = ctk.CTkLabel(
                        icon_frame,
                        text="📊",
                        font=(FONTS['icon'], 28),  # تصغير حجم الخط للأيقونات الافتراضية
                        text_color="white"
                    )
            else:
                # أيقونة افتراضية
            ...
            icon_label.pack(pady=(30, 10))  # تحسين المسافات مع الأيقونات الأصغر

            # النص أسفل الأيقونة مع وضوح عالي
            text_label = ctk.CTkLabel(
                icon_frame,
                text=text,
                font=(FONTS['arabic'], 18, "bold"),  # تكبير الخط أكثر
                text_color="#FFFFFF",  # أبيض ناصع
                wraplength=120,
                justify="center"
            )
            text_label.pack(pady=(0, 20))  # زيادة المسافة السفلية لتتناسب مع الشريط الأكبر

            # إضافة ظل للنص لمزيد من الوضوح (إذا كان مدعوماً)
            try:
                text_label._apply_appearance_mode()
                text_label._canvas.create_text(
                    62, 52,  # تقريباً منتصف الإطار
                    text=text,
            ...
            # جعل الأيقونة قابلة للنقر - مع ربط خاص للموظفين
            if text == "الموظفين":
                command = self.show_employees  # استخدام النظام الموحد المطور
            elif text == "المحاسبة":
                command = self.open_accounting
            elif text == "الحسابات":
            ...
            # تأثير التمرير مع إضاءة محسنة
            def on_enter(e, frame=icon_frame, label=text_label):
                frame.configure(fg_color="#1B5E20", corner_radius=10)
                label.configure(text_color="#F0F8FF")  # أبيض مضيء أكثر عند التمرير

            def on_leave(e, frame=icon_frame, label=text_label):
                frame.configure(fg_color="transparent", corner_radius=0)
                label.configure(text_color="#FFFFFF")  # العودة للأبيض العادي

            icon_frame.bind("<Enter>", lambda e: on_enter(e, icon_frame, text_label))
            icon_frame.bind("<Leave>", lambda e: on_leave(e, icon_frame, text_label))
            icon_label.bind("<Enter>", lambda e: on_enter(e, icon_frame, text_label))
            icon_label.bind("<Leave>", lambda e: on_leave(e, icon_frame, text_label))
            text_label.bind("<Enter>", lambda e: on_enter(e, icon_frame, text_label))
            text_label.bind("<Leave>", lambda e: on_leave(e, icon_frame, text_label))
    ...
    def create_main_grid_area(self, parent):
        """إنشاء منطقة الشبكة الرئيسية للأيقونات مطابقة للصورة تماماً"""
        # إطار رئيسي بخلفية رمادية داكنة مثل الصورة
        main_grid_frame = ctk.CTkFrame(parent, fg_color="#3C3C3C", corner_radius=0)
        main_grid_frame.pack(fill="both", expand=True, pady=(0, 0))
        ...
        first_row = ctk.CTkFrame(grid_container, fg_color="transparent")
        first_row.pack(fill="x", pady=10)

        first_row_icons = [
            ("assets/icons/16.ico", "أهلاً بكم", "#5DADE2"),
            ("assets/icons/53.ico", "إعداد", "#5DADE2"),
            ("assets/icons/1.ico", "إدخال الأصناف", "#4ECDC4"),
            ("assets/icons/4.ico", "إدخال الحسابات", "#F39C12"),
            ("assets/icons/24.ico", "الحركة اليومية", "#8E44AD"),
            ("assets/icons/52.ico", "تحليل المبيعات", "#3498DB")
        ]

        for icon_path, text, color in first_row_icons:
            self.create_modern_grid_button(first_row, icon_path, text, color, self.get_command_for_button(text))

        # الصف الثاني (6 أيقونات)
        second_row = ctk.CTkFrame(grid_container, fg_color="transparent")
        second_row.pack(fill="x", pady=10)

        second_row_icons = [
            ("assets/icons/31.ico", "مخزن", "#F39C12"),
            ("assets/icons/54.ico", "بيع", "#27AE60"),
            ("assets/icons/18.ico", "شراء", "#E74C3C"),
            ("assets/icons/17.ico", "صرف", "#E67E22"),
            ("assets/icons/44.ico", "مؤشرات", "#16A085"),
            ("assets/icons/4.ico", "مرتجع بيع", "#27AE60")
        ]

        for icon_path, text, color in second_row_icons:
            self.create_modern_grid_button(second_row, icon_path, text, color, self.get_command_for_button(text))

        # الصف الثالث (6 أيقونات)
        third_row = ctk.CTkFrame(grid_container, fg_color="transparent")
        third_row.pack(fill="x", pady=10)

        third_row_icons = [
            ("assets/icons/11.ico", "عرض أسعار", "#16A085"),
            ("assets/icons/23.ico", "مرتجع شراء", "#8E44AD"),
            ("assets/icons/30.ico", "كمية", "#9B59B6"),
            ("assets/icons/29.ico", "تحويل لمخزن", "#3498DB"),
            ("assets/icons/21.ico", "تسوية مخزن", "#1ABC9C"),
            ("assets/icons/52.ico", "مؤشرات", "#16A085")
        ]

        for icon_path, text, color in third_row_icons:
            self.create_modern_grid_button(third_row, icon_path, text, color, self.get_command_for_button(text))

    def get_command_for_button(self, text):
        """الحصول على الأمر المناسب لكل زر"""
        commands = {
            "أهلاً بكم": self.show_welcome,
            "إعداد": self.show_settings,
            "إدخال الأصناف": self.show_items_entry,
            "إدخال الحسابات": self.show_accounts_entry,
            "الحركة اليومية": self.show_daily_movement,
            "تحليل المبيعات": self.show_sales_analysis,
            "مخزن": self.show_warehouse,
            "بيع": self.show_sales,
            "شراء": self.show_purchases,
            "صرف": self.show_expenses,
            "مؤشرات": self.show_indicators,
            "مرتجع بيع": self.show_sales_return,
            "عرض أسعار": self.show_price_quote,
            "مرتجع شراء": self.show_purchase_return,
            "كمية": self.show_quantity,
            "تحويل لمخزن": self.show_warehouse_transfer,
            "تسوية مخزن": self.show_inventory_adjustment
        }
        return commands.get(text, self.show_default)

    def create_modern_grid_button(self, parent, icon_path, text, color, command):
        """إنشاء زر حديث في الشبكة مطابق للصورة"""
        # إطار الزر بحجم مناسب
        button_frame = ctk.CTkFrame(
            parent,
            width=140,  # حجم معقول ومتوازن
            height=140,  # حجم مربع متوازن
            fg_color=color,
            corner_radius=15
        )
        ...
        # تحميل الأيقونة
        if os.path.exists(icon_path):
            icon_image = self.load_icon_image(icon_path, (75, 75))  # حجم معقول ومحسن
            if icon_image:
                icon_label = ctk.CTkLabel(
                    button_frame,
                    image=icon_image,
                ...
                icon_label = ctk.CTkLabel(
                    button_frame,
                    text="📊",
                    font=(FONTS['icon'], 26),  # حجم معقول ومحسن
                    text_color="white",
                    fg_color="transparent"
                )
                icon_label.pack(pady=(15, 5))
        ...
        text_label = ctk.CTkLabel(
            button_frame,
            text=text,
            font=(FONTS['arabic'], 12, "bold"),  # حجم معقول ومتوازن
            text_color="white",
            wraplength=100,
            justify="center",
            fg_color="transparent"
    ...
    def open_default(self):
        """دالة افتراضية للأيقونات"""
        messagebox.showinfo("معلومات", "هذه الميزة قيد التطوير")

    # دوال الأزرار الجديدة مطابقة للصورة
    def show_welcome(self):
        """عرض صفحة الترحيب"""
        messagebox.showinfo("أهلاً بكم", "مرحباً بكم في برنامج ست الكل للمحاسبة")

    def show_settings(self):
        """عرض صفحة الإعدادات"""
        messagebox.showinfo("إعداد", "صفحة الإعدادات قيد التطوير")

    def show_items_entry(self):
        """عرض صفحة إدخال الأصناف"""
        messagebox.showinfo("إدخال الأصناف", "صفحة إدخال الأصناف قيد التطوير")

    def show_accounts_entry(self):
        """عرض صفحة إدخال الحسابات"""
        messagebox.showinfo("إدخال الحسابات", "صفحة إدخال الحسابات قيد التطوير")

    def show_daily_movement(self):
        """عرض صفحة الحركة اليومية"""
        messagebox.showinfo("الحركة اليومية", "صفحة الحركة اليومية قيد التطوير")

    def show_sales_analysis(self):
        """عرض صفحة تحليل المبيعات"""
        messagebox.showinfo("تحليل المبيعات", "صفحة تحليل المبيعات قيد التطوير")

    def show_warehouse(self):
        """عرض صفحة المخزن"""
        messagebox.showinfo("مخزن", "صفحة المخزن قيد التطوير")

    def show_expenses(self):
        """عرض صفحة الصرف"""
        messagebox.showinfo("صرف", "صفحة الصرف قيد التطوير")

    def show_indicators(self):
        """عرض صفحة المؤشرات"""
        messagebox.showinfo("مؤشرات", "صفحة المؤشرات قيد التطوير")

    def show_sales_return(self):
        """عرض صفحة مرتجع البيع"""
        messagebox.showinfo("مرتجع بيع", "صفحة مرتجع البيع قيد التطوير")

    def show_price_quote(self):
        """عرض صفحة عرض الأسعار"""
        messagebox.showinfo("عرض أسعار", "صفحة عرض الأسعار قيد التطوير")

    def show_purchase_return(self):
        """عرض صفحة مرتجع الشراء"""
        messagebox.showinfo("مرتجع شراء", "صفحة مرتجع الشراء قيد التطوير")

    def show_quantity(self):
        """عرض صفحة الكمية"""
        messagebox.showinfo("كمية", "صفحة الكمية قيد التطوير")

    def show_warehouse_transfer(self):
        """عرض صفحة تحويل المخزن"""
        messagebox.showinfo("تحويل لمخزن", "صفحة تحويل المخزن قيد التطوير")

    def show_inventory_adjustment(self):
        """عرض صفحة تسوية المخزن"""
        messagebox.showinfo("تسوية مخزن", "صفحة تسوية المخزن قيد التطوير")

    def show_default(self):
        """دالة افتراضية"""
        messagebox.showinfo("معلومات", "هذه الميزة قيد التطوير")

    # دوال القائمة العلوية الجديدة
    def show_activation(self):
        """عرض صفحة التنشيط"""
        messagebox.showinfo("تنشيط", "صفحة التنشيط قيد التطوير")

    def show_customer_service(self):
        """عرض صفحة خدمة العملاء"""
        messagebox.showinfo("خدمة العملاء", "صفحة خدمة العملاء قيد التطوير")

    def show_centers(self):
        """عرض صفحة المراكز"""
        messagebox.showinfo("المراكز", "صفحة المراكز قيد التطوير")

    def show_reports(self):
        """عرض صفحة التقارير"""
        messagebox.showinfo("التقارير", "صفحة التقارير قيد التطوير")

    def show_treasury(self):
        """عرض صفحة الخزينة"""
        messagebox.showinfo("الخزينة", "صفحة الخزينة قيد التطوير")

    def show_program_menu(self):
        """عرض قائمة البرنامج"""
        messagebox.showinfo("برنامج", "قائمة البرنامج قيد التطوير")

    def create_grid_button(self, parent, icon, text, color, command, icon_path=None, font_size='large', top_padding=15):
        """إنشاء زر في الشبكة (الدالة القديمة للتوافق)"""
    ...
    def load_icon_image(self, icon_path: str, size: tuple = (160, 160)) -> Optional[Any]:
        ...
        Load icon image from file with comprehensive error handling and ICO support
        ...
        try:
            ...
            if hasattr(self, 'loaded_images') and cache_key in self.loaded_images:
            ...
            with Image.open(icon_path) as image:
                ...
                if icon_path.lower().endswith('.ico'):
                    ...
                    if hasattr(image, 'size') and image.size[0] > 0:
                        ...
                        print("تحذير: ملف ICO قد يكون تالف")
                        return None
                else:
                    # الحفاظ على الشفافية للملفات الأخرى
                    if image.mode != 'RGBA':
                        image = image.convert('RGBA')

                # إزالة الخلفية البيضاء للملفات غير ICO
    ...
    def show_main_dropdown_menu(self):
        ...
        try:
            ...
            try:
                ...
                dropdown_window.geometry(f"420x520+{dropdown_x}+{dropdown_y}")  # زيادة العرض والارتفاع (+40px عرض، +40px ارتفاع)

            except Exception as e:
                print(f"تحذير: فشل في تحديد موضع النافذة: {e}")
                # موضع افتراضي في وسط الشاشة مع الحجم المحسن
                dropdown_window.geometry("420x520+100+100")
    ...
    def show_simple_main_menu(self):
        ...
        try:
            ...
            simple_window.geometry(f"450x650+{x}+{y}")  # زيادة الحجم (+50px عرض، +50px ارتفاع)
    ...
    def show_employees(self):
        """عرض الموظفين - النظام الموحد الجديد"""
        try:
            # استخدام النظام الموحد الجديد
            from ui.views.unified_employees_management import UnifiedEmployeesManagement
            unified_employees = UnifiedEmployeesManagement(self.main_window)
            print("✅ تم فتح نظام إدارة الموظفين الموحد والمتطور")
        except ImportError:
            # استخدام النافذة القديمة كبديل
            try:
                from ui.employees_window import EmployeesWindow
                employees_window = EmployeesWindow(self.main_window)
                print("✅ تم فتح نافذة الموظفين الأساسية")
            except Exception as e:
    ...
    def open_employees(self):
        """فتح الموظفين - النظام الموحد الجديد"""
        try:
            # استخدام النظام الموحد الجديد
            from ui.views.unified_employees_management import UnifiedEmployeesManagement
            unified_employees = UnifiedEmployeesManagement(self.main_window)
